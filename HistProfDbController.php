...
use App\Models\HistoricoProfissionaisUnidadeModels;
use App\Models\VHistCadProfModels;
use Exception;

class HistProfDbController extends BaseController
{
    // private $ModelUpload;
    private $ModelsHistoricoProfissionaisUnidade;
    private $ModelsVHistCadProf;
    private $message;
    private $uri;
    private $pagination;
    ...
    public function __construct()
    {
        ...
        $this->ModelsHistoricoProfissionaisUnidade = new HistoricoProfissionaisUnidadeModels();
        $this->uri = new \CodeIgniter\HTTP\URI(current_url());
        $this->ModelsVHistCadProf = new VHistCadProfModels();
    ...
    public function dbFields($processRequestFields = array())
    {
        ...
        $dbCreate = array();
        $autoColumn = $this->ModelsHistoricoProfissionaisUnidade->getColumnsFromTable();
    ...
    public function dbFieldsFilter($processRequestFields = array())
    {
        ...
        $dbCreate = array();
        $autoColumn = $this->ModelsVHistCadProf->getColumnsFromTable();
        ...
        (isset($processRequestFields['assinatura'])) ? ($dbCreate['hpu_assinatura'] = $processRequestFields['assinatura']) : (NULL);
    ...
    public function dbCreate($parameter = NULL)
    {
        try {
            ...
            // myPrint('$parameter :: src\app\Controllers\HistProfDbController.php', $parameter, true);
            $resultDb = $this->ModelsHistoricoProfissionaisUnidade->dbCreate($parameter);
            // myPrint('$parameter :: src\app\Controllers\HistProfDbController.php', $resultDb);
            $affectedRows = $this->ModelsHistoricoProfissionaisUnidade->affectedRows();
            if ($affectedRows > 0) {
                $dbCreate['insertID'] = $this->ModelsHistoricoProfissionaisUnidade->insertID();
    ...
    public function dbRead($parameter = NULL, $page = 1, $limit = 10)
    {
        ...
        try {
            if ($parameter !== NULL) {
                $dbResponse = $this
                    ->ModelsHistoricoProfissionaisUnidade
                    ->where('id', $parameter)
    ...
    {
        try {
            $this->ModelsHistoricoProfissionaisUnidade->dbDelete('id', $parameter);
            $affectedRows = $this->ModelsHistoricoProfissionaisUnidade->affectedRows();
            if ($affectedRows > 0) {
