...
class TesteAnonimoApiController extends ResourceController
{
    use ResponseTrait;
    ...
    # route POST /www/index.php/fia/ptpa/anonimo/api/indicadores/(:any)
    # route GET /www/index.php/fia/ptpa/anonimo/api/indicadores/(:any)
    # Informação sobre o controller
    # retorno do controller [JSON]
    public function create_update($parameter = NULL)
    {
        ...
        if (isset($processRequest['valor'])) {
            try {
                
                $processRequest['valor'] = criptografar($processRequest['valor']);

            } catch (Exception $e) {
                // Em caso de erro na criptografia
                return $this->response->setStatusCode(500)->setJSON([
                    'status' => 'error',
                    'message' => 'Erro ao criptografar dados: ' . $e->getMessage()
                ]);
            }
        }
        myPrint('$processRequest :: ', $processRequest);

        // criptografar($apiRespond);
        // ehTokenCriptografado($apiRespond);
        // decriptografar($apiRespond);
    }
