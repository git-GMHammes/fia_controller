...
use App\Models\EscolaridadeModels;
use App\Models\VUnidadesMunicipiosModels;
use Exception;

class EscolaridadeDbController extends BaseController
{
    // private $ModelUpload;
    ...
    public function __construct()
    {
        ...
        $this->ModelEscolaridade = new EscolaridadeModels();
    ...
    public function dbFields($processRequestFields = array())
    {
        ...
        $dbCreate = array();
        $autoColumn = $this->ModelEscolaridade->getColumnsFromTable();
    ...
    public function dbCreate($parameter = NULL)
    {
        try {
            ...
            $this->ModelEscolaridade->dbCreate($parameter);
            $affectedRows = $this->ModelEscolaridade->affectedRows();
            if ($affectedRows > 0) {
                $dbCreate['insertID'] = $this->ModelEscolaridade->insertID();
    ...
    public function dbUpdate($key, $parameter = NULL)
    {
        ...
        try {
            if (
                ...
                $this->ModelEscolaridade->dbUpdate($key, $parameter);
            } else {
                // exit('Não quero aqui');
                $this->ModelEscolaridade->dbUpdate($key, $this->dbFields($parameter));
            }
            #
            $affectedRows = $this->ModelEscolaridade->affectedRows();
            #
    ...
    {
        try {
            $this->ModelEscolaridade->dbDelete('id', $parameter);
            $affectedRows = $this->ModelEscolaridade->affectedRows();
            if ($affectedRows > 0) {
