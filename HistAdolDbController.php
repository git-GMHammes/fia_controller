...
use App\Models\VHistCadAdolModels;
use App\Models\HistoricoAdolecenteUnidadeModels;
use Exception;

class HistAdolDbController extends BaseController
{
    // private $ModelUpload;
    private $ModelsHistoricoAdolecenteUnidade;
    private $ModelsVHistCadAdol;
    private $message;
    private $uri;
    private $pagination;
    ...
    public function __construct()
    {
        ...
        $this->ModelsHistoricoAdolecenteUnidade = new HistoricoAdolecenteUnidadeModels();
        $this->ModelsVHistCadAdol = new VHistCadAdolModels();
    ...
    public function dbFields($processRequestFields = array())
    {
        ...
        $dbCreate = array();
        $autoColumn = $this->ModelsHistoricoAdolecenteUnidade->getColumnsFromTable();
    ...
    public function dbFieldsFilter($processRequestFields = array())
    {
        // myPrint($processRequestFields, 'src\app\Controllers\SystemUploadDbController.php', true);
        $dbCreate = array();
        $autoColumn = $this->ModelsVHistCadAdol->getColumnsFromTable();
    ...
    {
        try {
            $this->ModelsHistoricoAdolecenteUnidade->dbCreate($this->dbFields($parameter));
            $affectedRows = $this->ModelsHistoricoAdolecenteUnidade->affectedRows();
            if ($affectedRows > 0) {
                $dbCreate['insertID'] = $this->ModelsHistoricoAdolecenteUnidade->insertID();
    ...
    public function dbRead($parameter = NULL, $page = 1, $limit = 10)
    {
        ...
        try {
            if ($parameter !== NULL) {
                $dbResponse = $this
                    ->ModelsHistoricoAdolecenteUnidade
                    ->where('id', $parameter)
    ...
    public function dbUpdate($key, $parameter = NULL)
    {
        if (
            ...
            $this->ModelsHistoricoAdolecenteUnidade->dbUpdate($key, $parameter);
        } else {
            $this->ModelsHistoricoAdolecenteUnidade->dbUpdate($key, $this->dbFields($parameter));
        }
        try {
            $affectedRows = $this->ModelsHistoricoAdolecenteUnidade->affectedRows();
            if ($affectedRows > 0) {
    ...
    {
        try {
            $this->ModelsHistoricoAdolecenteUnidade->dbDelete('id', $parameter);
            $affectedRows = $this->ModelsHistoricoAdolecenteUnidade->affectedRows();
            if ($affectedRows > 0) {
