...
use App\Controllers\CargoFuncaoDbController;
// use App\Controllers\SystemUploadDbController;

use Exception;

class CargoFuncaoApiController extends ResourceController
{
    use ResponseTrait;
    ...
    public function create_update($parameter = NULL)
    {
        ...
        if ($json) {
            ...
            return redirect()->to('fia/ptpa/cargofuncao/endpoint/exibir');
        }
    }

    # route POST /index.php/fia/ptpa/cargofuncao/api/exibir/(:any)
    # route GET /index.php/fia/ptpa/cargofuncao/api/exibir/(:any)
    # Informação sobre o controller
    # retorno do controller [JSON]
    public function dbRead($parameter = NULL)
    {
        ...
        try {
            ...
            // myPrint($requestDb, 'src\app\Controllers\CargoFuncaoApiController.php');
            #
            $apiRespond = [
                'status' => 'success',
    ...
    # route POST /# www/index.php/fia/ptpa/CargoFuncao/api/filtrar/(:any)
    # route GET /# www/index.php/fia/ptpa/CargoFuncao/api/filtrar/(:any)
    # Informação sobre o controller
    # retorno do controller [JSON]
    ...
    public function dbDelete($parameter1 = NULL, $parameter2 = NULL)
    {
        ...
        try {
            if (
                ...
                // myPrint($parameter1, 'src\app\Controllers\CargoFuncaoApiController.php, 271');
                $dbResponse = $this->DbController->dbDelete($parameter1);
                $this->message->message(['Cargo/Função Eliminada com sucesso.'], 'success', $dbUpdate = array(), 5);
            } elseif (
                $parameter1 !== NULL
                ...
                // myPrint($dbUpdate, 'src\app\Controllers\CargoFuncaoApiController.php', true);
                $dbResponse = $this->DbController->dbUpdate($parameter1, $dbUpdate);
                $this->message->message(['Cargo/Função Restaurada com sucesso.'], 'success', $dbUpdate = array(), 5);
            } elseif (
                $parameter1 !== NULL
            ) {
                // myPrint($parameter1, 'src\app\Controllers\CargoFuncaoApiController.php, 287');
                $dbUpdate = array(
                    'deleted_at' => date('Y-m-d H:i:s')
                );
                // myPrint($dbUpdate, 'dbDelete/CargoFuncao');
                $dbResponse = $this->DbController->dbUpdate($parameter1, $dbUpdate);
                $this->message->message(['Cargo/Função Excluída com sucesso.'], 'success', $dbUpdate = array(), 5);
            } else {
                $this->message->message(['Erro ao Excluir a Cargo/Função.'], 'warning', $dbUpdate = array(), 5);
            }
            $apiRespond = [
                'status' => 'success',
                'message' => 'API loading data (dados para carregamento da API)',
    ...
    public function dbCleaner($parameter = NULL)
    {
        ...
        try {
            ...
            // myPrint($dbResponse, 'src\app\Controllers\CargoFuncaoApiController.php');
            $apiRespond = [
                'status' => 'success',
                'message' => 'API loading data (dados para carregamento da API)',
                'date' => date('Y-m-d'),
